...
void ChipPhysics::setupBodies()
{
    ...
    b2Vec2 vertices[4];
    vertices[0].Set(0, 0);
    vertices[1].Set(101, 0);//101 is the label width 211 is height
    vertices[2].Set(101, 211);
    vertices[3].Set(0, 211);
    ...
    boundryBodyDef.position.Set(50, 400);
...
void ChipPhysics::createChips(int betAmt)//bet amt will determine number/color of chips
{
    for(int i = 0; i < betAmt/5; i++)//one chip for every $5 bet
    {
        ...
        chipBodyDef.position.Set(50.0f + i * 25, 20.0f); //Starting position of the body (i*25 is an offset for creating multiple chips to avoid overlap)
